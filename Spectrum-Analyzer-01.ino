...
void setup()
{
  ...
  display.begin(SSD1306_SWITCHCAPVCC,0x3D);           //begin OLED @ hex addy 0x3C
...
void loop()
{
  ...
  for (i = 0; i < 64; i++) {                         //take 128 samples
    val = analogRead(ANALOG_IN_PIN);                             //get audio from Analog 0
    data[i] = val / 4 - 64;                          //each element of array is val/4-128
    im[i] = 0;                                        //
